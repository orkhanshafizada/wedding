...
class UpdateMenuRequest extends FormRequest
{
    public function authorize(): bool
    {
        ...
        return $user instanceof User
            ...
            && app(AdminAccessService::class)->can($user, 'menu.edit', $menu);
    }
    ...
    public function rules(): array
    {
        ...
        return [
            ...
            'keep_main_image' => ['sometimes', 'boolean'],
    ...
    protected function prepareForValidation(): void
    {
        ...
        $this->merge([
            ...
            'keep_main_image' => $this->boolean('keep_main_image', true),
    ...
    public function withValidator($validator): void
    {
        $validator->after(function ($validator): void {
            ...
            foreach ($translations as $index => $row) {
                ...
                if ($link !== '' && $this->linkExists($link, $menuId)) {
                    $validator->errors()->add(
            ...
            foreach ((array) $this->input('included_items', []) as $index => $item) {
                ...
                if ($type === MenuIncludedItemType::MENU->value && $id === $menuId) {
                    $validator->errors()->add('included_items.' . $index . '.id', __('A menu cannot include itself.'));
                }
    ...
    private function allowedTypeValues(): array
    {
        ...
        $values = $accessService->allowedMenuTypeValues($user);

        if ($menu instanceof Menu) {
            $currentType = $menu->type instanceof \BackedEnum ? (string) $menu->type->value : (string) $menu->type;

            if (trim($currentType) !== '') {
                $values[] = trim($currentType);
            }
        }
